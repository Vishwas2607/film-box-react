...
export default function GenerateCard({Title,Year,imdbID,Type,Poster}: Movie) {
    ...
    const classes = clsx(
        "absolute top-0 left-0 inline-flex justify-center items-center text-2xl md:text-4xl rounded-full w-[35px] h-[35px] md:w-[50px] md:h-[50px]"
    )

    const cardClass = clsx(
        "relative border-2 w-[150px] md:w-[220px] h-[250px] md:h-[350px] flex flex-col hover:shadow-lg transition duration-500 ease-in-out shadow-gray-600 animate-fade-in justify-between pb-2 cursor-pointer"
    );

    const typeClass = clsx(
        "capitalize border-2 rounded-xl px-1 md:px-2 py-0.5 md:py-1 text-[10px] md:text-sm transition duration-500 ease-in-out",
    ...
    return (
        <div className={active && !isSaved ? `${cardClass} animate-shrink-fade` : cardClass} onClick={()=>navigate(`/moviedetails/${imdbID}`)}>
            ...
            <img src={Poster} alt={`${Title}'s Poter`} title={`Poster of ${Title}`} aria-label="Movie Poster" loading="lazy" className=" h-[150px] md:h-[250px] border-2 border-black text-center grid place-items-center"/>
            ...
            <div className="flex flex-wrap w-full px-2 items-center justify-between">
                ...
                <p className="capitalize border-2 border-gray-400 rounded-xl px-1 md:px-2 py-1 text-[10px] md:text-sm truncate md:line-clamp-none max-w-[85px] md:max-w-fit">Year: {Year}</p>
            </div>
        </div>
    )
}